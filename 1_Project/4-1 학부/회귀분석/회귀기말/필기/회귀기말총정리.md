...
## 14. Correlated Errors (Autocorrelation)
### Definition & Consequences
* **가정 위배 (Assumption Violation):** 오차항들 간에 상관관계가 존재함. $Cov(\epsilon_i, \epsilon_j) \neq 0, \quad i \neq j$.
...
* **변수 변환 (Transformation) 아이디어:**
    ...
    * $t-1$ 시점 모델에 $\rho$ 곱함:
    $\rho Y_{t-1} = \rho \beta_0 + \rho \beta_1 X_{t-1} + \rho \epsilon_{t-1}$
    * 두 식을 뺌 (Difference):
        * $(Y_t - \rho Y_{t-1}) = \beta_0(1-\rho) + \beta_1(X_t - \rho X_{t-1})$
        $+ (\underbrace{\epsilon_t - \rho \epsilon_{t-1}}_{=u_t})$
    * 변환된 변수 
    $Y'_t = Y_t - \rho Y_{t-1}, X'_t = X_t - \rho X_{t-1}$
    에 대해 OLS 수행.
