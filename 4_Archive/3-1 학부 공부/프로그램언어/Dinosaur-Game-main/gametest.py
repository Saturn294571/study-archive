...
class Dino(pygame.sprite.Sprite):  # 플레이어 캐릭터(공룡) 클래스
    ...
    def jump(self):  # 점프 동작
        ...
        if self.rect.centery >= 360:  # 지면에 있는 경우에만 점프 가능
            while self.rect.centery - self.velocity > 40:  # 일정 높이까지 상승
                self.rect.centery -= 1

    def duck(self):  # 숙이는 동작
        self.ducking = True
        self.rect.centery = 380  # 공룡의 y 위치를 조정하여 숙인 상태로 만듦

    def unduck(self):  # 숙인 상태 해제
        self.ducking = False
        self.rect.centery = 360  # 공룡을 원래 위치로 복구

    def apply_gravity(self):  # 중력 효과 적용
        if self.rect.centery <= 360:  # 지면 위에 있을 때 아래로 떨어짐
...
class Ptero(pygame.sprite.Sprite):  # 장애물(익룡) 클래스
    def __init__(self):
        ...
        self.y_pos = random.choice([280, 295, 350])  # 익룡의 랜덤 y 위치
        self.sprites = [
...
ground_rect = ground.get_rect(center=(640, 400))  # 지면 위치 설정
...
while True:
    ...
    if not game_over:  # 게임 진행 중일 경우
        ...
        screen.blit(ground, (ground_x, 360))  # 지면 표시
        screen.blit(ground, (ground_x + 1280, 360))  # 두 번째 지면 표시 (순환)
